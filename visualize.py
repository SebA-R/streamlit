...
class visualize:
    ...
    def visualize_area_plot(df, columns):
        st.subheader("Area Plot")
        for column in columns:
            if pd.api.types.is_numeric_dtype(df[column]):
                st.write(f"### {column} Area Plot")
                plt.figure(figsize=(8, 6))
                plt.fill_between(df["Name"], df[column], alpha=0.7)
                plt.xlabel("Name")
                plt.ylabel(column)
                plt.xticks(rotation=90)
                plt.tight_layout()
    ...
    def visualize_stacked_bar_plot(df, columns):
        st.subheader("Stacked Bar Plot")
        plt.figure(figsize=(8, 6))
        bottom = None
        for column in columns:
            if pd.api.types.is_numeric_dtype(df[column]):
                plt.bar(df["Name"], df[column], bottom=bottom, alpha=0.7, label=column)
                if bottom is None:
                    bottom = df[column]
                else:
                    bottom += df[column]
